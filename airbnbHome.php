...
if(isset($_POST['submit'])){
    ...
    switch ($search_item) {
        ...
        case 'forest':
            ...
            exit();
        case 'mountain':
            header("Location: mountain.php");
            exit();
        case 'city':
            header("Location: city.php");
            exit();
        default:
            $select_airbnb = mysqli_query($con, "SELECT * FROM `airbnbs` WHERE name LIKE '%$search_item%'") or die('query failed');
            if($select_airbnb && mysqli_num_rows($select_airbnb) > 0){
                // Store the fetched results in an array to use later
                $airbnb_results = [];
                while ($fetch_airbnb = mysqli_fetch_assoc($select_airbnb)) {
                    // Collect the Airbnb information into an array
                    $airbnb_results[] = $fetch_airbnb;
                }
                // Store the Airbnb results in a session variable
                session_start();
                $_SESSION['airbnb_results'] = $airbnb_results;
...
<body onload="clearSearch()">
  <header>
    <nav>
      <ul class="nav-bar">
        ...
        <span class="menu">
          <li><b><a href="airbnbHome.php">home</a></b></li>
  ...
  <script>
    function clearSearch() {
      document.querySelector('.box').value = ''; // Clears the search input field
    }
  </script>
